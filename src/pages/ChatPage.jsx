...
const DEVICE_CATALOG = {
  ...
  MOTHERBOARD: [
    ...
    { name: 'ASRock B550 Steel Legend', brand: 'ASRock' },
  ],
  PSU: [
...
function getQuickActionQuery(questionId, deviceName) {
  const map = {
    ...
    gaming: `How does the ${deviceName} perform for gaming?`,
    gaming_1080p: `How does the ${deviceName} perform at 1080p gaming?`,
    gaming_1440p: `How does the ${deviceName} perform at 1440p gaming?`,
    gaming_4k: `How does the ${deviceName} perform at 4K gaming?`,
    productivity: `How does the ${deviceName} perform for productivity tasks?`,
    ...
    compatible_ram: `What RAM type and speed does the ${deviceName} support?`,
    ...
    features: `What are the key features of the ${deviceName}?`,
    efficiency: `What is the efficiency rating of the ${deviceName}?`,
    vs_competitor: `Compare the ${deviceName} against its main competitors in the Philippines.`,
    worth_buying: `Is the ${deviceName} worth buying in the Philippines in 2025?`,
  };
        ...
        <div style={{ padding: '16px 20px 0', borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ position: 'relative', marginBottom: '12px' }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search models..."
              style={{ width: '100%', background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: '10px', padding: '8px 12px', fontSize: '13px', color: 'var(--text)', fontFamily: 'inherit', boxSizing: 'border-box', outline: 'none' }} autoFocus />
          </div>
          <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '12px' }}>
            {CATALOG_TABS.map(tab => (
              <button key={tab} onClick={() => { setActiveTab(tab); setSearch(''); }}
                style={{ flexShrink: 0, background: activeTab === tab ? 'var(--accent)' : 'var(--bg-3)', border: 'none', borderRadius: '20px', padding: '5px 12px', fontSize: '11px', fontWeight: 600, color: activeTab === tab ? 'white' : 'var(--text-3)', cursor: 'pointer', fontFamily: 'inherit' }}>
                {TAB_LABELS[tab]}
              </button>
            ))}
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '12px' }}>
              {items.map((device, i) => (
                <button key={i} onClick={() => onSelect(device.name)}
                  style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: '12px', padding: 0, cursor: 'pointer', overflow: 'hidden', textAlign: 'left' }}>
                  <div style={{ width: '100%', aspectRatio: '1', overflow: 'hidden', position: 'relative' }}>
                    <DeviceImage device={device} category={activeTab} />
                  </div>
                  <div style={{ padding: '8px 10px 10px' }}>
                    <div style={{ fontSize: '10px', color: 'var(--accent)', fontWeight: 600, marginBottom: '2px', textTransform: 'uppercase' }}>{device.brand}</div>
  ...
  return (
    <button onClick={onClick}
      ...
      <div style={{ padding: '10px 14px', flex: 1 }}>
        <div style={{ fontSize: isPrimary ? '15px' : '14px', color: isPrimary ? 'white' : 'var(--text)', fontWeight: isPrimary ? 700 : 500, lineHeight: 1.3 }}>{name}</div>
      </div>
    </button>
  );
}
  ...
  return (
    <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginTop: '4px' }}>
      ...
      <div style={{ borderTop: '1px solid var(--border)', padding: '10px 18px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <button onClick={onUploadAgain} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '10px', padding: '11px 16px', fontSize: '13px', color: 'var(--text-3)', cursor: 'pointer', fontFamily: 'inherit' }}>Upload Again</button>
      </div>
    </div>
  );
}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {step.options.map(opt => (
            <button key={opt.id} onClick={() => onSelect(opt.id, opt.label)}
              style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 16px', fontSize: '14px', color: 'var(--text)', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', fontWeight: 500 }}>
              {opt.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => onSelect(s)}
              style={{ background: i === 0 ? 'var(--accent)' : 'var(--bg-3)', border: i === 0 ? 'none' : '1px solid var(--border)', borderRadius: '10px', padding: '13px 16px', fontSize: i === 0 ? '15px' : '14px', color: i === 0 ? 'white' : 'var(--text)', cursor: 'pointer', textAlign: 'center', fontFamily: 'inherit', fontWeight: i === 0 ? 700 : 500 }}>
              {s}
            </button>
          ))}
          <button onClick={onSeeMore} style={{ background: 'none', border: '1px dashed var(--border)', borderRadius: '10px', padding: '10px 16px', fontSize: '13px', color: 'var(--accent)', cursor: 'pointer', textAlign: 'center', fontFamily: 'inherit', fontWeight: 600 }}>📦 See More Models</button>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
        {actions.map(q => (
          <button key={q.id} onClick={() => onSelect(q.id)}
            style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', borderRadius: '20px', padding: '6px 14px', fontSize: '12px', color: 'var(--text)', cursor: 'pointer', fontFamily: 'inherit' }}>
            {q.label}
          </button>
        ))}
      </div>
...
export default function ChatPage({ onLogout }) {
  ...
  const renderWizard = () => {
    ...
    if (wizardState.phase === 'steps') return (
      <WizardStepCard step={wizardState.steps[wizardState.stepIndex]} onSelect={handleWizardOption} onSkip={() => handleWizardOption('skipped', 'Skipped')} />
    );
    if (wizardState.phase === 'refined') return (
      <RefinedResultsCard reasoning={wizardState.reasoning} suggestions={wizardState.suggestions} onSelect={(name) => confirmDevice(name, wizardState.category)} onTypeMyself={() => { setWizardState(null); inputRef.current?.focus(); }} onSeeMore={() => openCatalog(wizardState.category)} />
    );
    if (wizardState.phase === 'quick_actions') return (
      <QuickActionCard deviceName={wizardState.deviceName} category={wizardState.category} onSelect={handleQuickAction} />
    );
    return null;
  };
  ...
  return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
              ...
              <div className="message-bubble">
                {msg.image && (
                  <div style={{ marginBottom: '10px' }}>
                    ...
                    <img src={`data:${msg.imageMime || 'image/jpeg'};base64,${msg.image}`} alt="Uploaded hardware" style={{ maxWidth: '200px', borderRadius: '8px', display: 'block' }} />
                  </div>
                )}
        ...
        <div style={{ width: '100%', maxWidth: '760px' }}>
          {uploadedPreview && (
            <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '10px 14px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <button type="button" onClick={() => { setUploadedFile(null); setUploadedPreview(null); if (fileRef.current) fileRef.current.value = ''; }}
                style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: '4px' }}>
      ...
      {catalogOpen && (
        <CatalogModal initialCategory={catalogCategory} onSelect={(name) => confirmDevice(name, catalogCategory)} onClose={() => setCatalogOpen(false)} />
      )}
    </div>
  );
}